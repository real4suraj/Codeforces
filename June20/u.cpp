...
void solve() {
    ...
    for(int i = 0; i < m; ++i){
        int x;
        cin >> x;
        if(Mp[x - 1].x == 0) continue;
        int pos = Mp[x - 1].x - 1;
        int prev = revMp[pos];
        int prevPos = Mp[x - 1].x;
        Mp[prev].x = pos + 1;
        Mp[prev].y = mp(Mp[prev].y.x, max(pos + 1, Mp[prev].y.y));

        Mp[x - 1].x = pos;
        Mp[x - 1].y = mp(min(pos, Mp[x - 1].y.x), Mp[x - 1].y.y);

        revMp[pos] = x - 1;
        revMp[prevPos] = prev;
    }
    for(auto x : Mp){
        cout << x.second.second.first + 1 << " " << x.second.second.second + 1<< endl;
    }
